<template>
	<div class="service-chat">
		...
		<div class="flex-start-center">
			<Input v-model="new_chat.chatid" placeholder="群名称或群链接" style="width: 300px"/>
...
export default {
	...
	methods:{
		...
		addChat(){
			if (!this.new_chat.chatid.trim()||!String(this.new_chat.type)||!String(this.new_chat.auth)) {
				return this.$Notice.error({title:'信息缺失'})
			}
			addChat(this.new_chat.chatid,this.new_chat.type,this.new_chat.auth).then((r)=>{
				this.getUserChat()
				this.new_chat.chatid = ''
				this.new_chat.type = ''
				this.new_chat.auth = ''
